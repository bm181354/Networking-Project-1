    ...
    try {
     ...
     // Using synchronized to stop any kind of error parallel threading;
      ...
      while (true) {
        ...
        if (msg.startsWith("@")){
          // private messages, split after the name of the user with delimeter white space (//s)
          String[] storeMessage = msg.split("\\s",2);
           // message shouldn't be null
          if(storeMessage[1]!= null ){
               
              if(!storeMessage[1].isEmpty())
              {

                 synchronized(this){
                  for(int index =0; index < maxUsersCount; index++){
                    if(threads[index] != null && threads[index].userName != null && threads[index].userName.equals(storeMessage[0]) && threads[index] != this){
                      // server echo the message to the sender and the particular reciever 
                      threads[index].output_stream.println("<" + name + "> " + storeMessage[1]);
                      this.output_stream.println("<" + name + "> " + storeMessage[1]);
                      // leave the loop 
                      break;
                    }
                  }
          ...
          // if user didn't include or send the message to particular user then echo the message to all connected user via Server
          // no particular order here. Thread index will point to particular user and then server will echo the message to all the 
          // connected user.
        } else{
          synchronized (this) {
            for (int i = 0; i < maxUsersCount;  i++) {
